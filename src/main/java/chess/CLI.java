        ...
        while (true) {
            ...
            if (input == null) {
                ...
                if (input.equals("help")) {
                    ...
                    for(Move move : gameState.getAllPlayerPossibleMoves()) {
                        writeOutput(move.getFrom().toString() + " " + move.getTo().toString());
                    }
                } else if (input.startsWith("move")) {
                    String[] parts = input.trim().split(" ");
                    if(parts.length == 3) {
                        String colrowFrom = parts[1].trim();
                        String colrowTo = parts[2].trim();

                        if(gameState.isDraw()) {
                            writeOutput("DRAW");
                        } else {
                            if (gameState.move(colrowFrom, colrowTo)) {
                                Player player = (gameState.getCurrentPlayer() == Player.Black) ? Player.White : Player.Black;
                                gameState.setCurrentPlayer(player);
                                if(gameState.isDraw() && gameState.isCheck()) {
                                    writeOutput("CHECKMATE");
                                    doNewGame();
                                }
                            } else {
                                writeOutput(colrowFrom + " -> " + colrowTo + " #incorrect step");
                            }
                        }
